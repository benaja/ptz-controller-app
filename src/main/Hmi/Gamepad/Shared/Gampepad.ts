...
import { EButtonDirection, IGamepadConfiguration } from './GamepadConfiguration';
...
import { InterfaceConfig } from '@main/store/userStore';
...
export abstract class Gamepad implements IHmi {
  ...
  protected log(toLog: string): void {
    console.log(`Gamepad:${toLog}`);
  }

  protected logError(toLog: string): void {
    console.error(`Gamepad:${toLog}`);
  }
  ...
  private printConnectionMessage(index: number, connection: number, onAir: boolean) {
