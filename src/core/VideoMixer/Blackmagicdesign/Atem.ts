...
export type AtemMixerConfig = z.infer<typeof atemMixerConfigSchema>;
...
export class Atem implements IVideoMixer {
  ...
  public readonly name = VideoMixerType.BlackmagicAtem;
  public readonly label = 'Atem';

  constructor(private config: AtemMixerConfig, private atemFactory: AtemFactory) {
  ...
  public async changeInput(newInput: number) {
    if (this.connection.connection) {
