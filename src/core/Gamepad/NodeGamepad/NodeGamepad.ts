...
export class NodeGamepad extends EventEmitter {
  ...
  public start(): void {
    ...
    this.log(
      `Starting connection procedure to device:${JSON.stringify(this.toIDeviceSpec(this.config))}`,
    );
  ...
  private connect(): void {
    ...
    try {
      this._usb = new HID(deviceToConnectTo.vendorId, deviceToConnectTo.productId);
      this.log('connected');
      this.emit('connected');
